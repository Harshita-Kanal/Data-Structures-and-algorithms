...
Sorting is arranging the elements of array in ascending order. It can be done by using three methods
•	Selection sort
•	Bubble sort
•	Insertion sort

Selection sort: In selection sort we find the minimum element in unsorted array and swap it with the element at the beginning of the array.

Bubble sort: In bubble sort we repeatedly swap two adjacent elements if they are in the wrong order.

Insertion sort: In insertion sort we insert an element from unsorted array to its correct position in sorted array.
