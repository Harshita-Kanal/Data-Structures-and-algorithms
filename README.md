...
It is frequently used in compeitive programming, and comes under number thoery section. The brute force method has a high complexity, therefore prime sieve is one easy way to deal with prime numbers, with a complexity of O(log(Log N)).It is also known as Sieve of Eratosthenes. 
In this algorithm, We take an array to mark if the number is prime or not as 1 or 0 respectively, then we mark all the odd numbers from 1 to n as prime(by preassuming). After it, we mark all the mutliples of initial odd numbers as not prime(that is 0 as they are a multiple already), for any number there has to be a multiple which is smaller than it's square root. 
For some numbers such as 0 and 1 we have to mark them as not prime by ourselves. The resulting array tells us if a number is prime or not.
