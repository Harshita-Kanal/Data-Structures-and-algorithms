...
## Heapsort
Heapsort is another interesting sorting algorithm that users can use to sort through any list. Heapsort uses the structure of a binary tree. Depending on the order of your nodes, the first value in your list (or the root node if you look at a tree) will be the smallest or largest value. Unlike a binary search tree, the two child nodes must either be smaller than or larger than the parent node depending on what the root value is. If the root (the parent) node has the smallest value, then the root's children must be bigger than the root. If the root has the biggest value, then the root's children must be smaller than the root. This concept applies the children nodes who have children. The main rule of thumb when using a heapsort algorithm is to make sure that your list maintains a binary tree structure.
